...
To install the chart with the release name `mongotest`:

```console
$ helm install --name mongotest -f values.yaml --namespace test .
```
...
| `installImage.tag`              | Image tag for the init container that establishes the replica set | 0.4                                      |
